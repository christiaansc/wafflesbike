...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'dni' => 'required|string|unique:productos,dni,'.$this->route('cliente')->id.'|max:8',
            'ruc' => 'required|string|unique:productos,ruc,'.$this->route('cliente')->id.'|max:11',         
            'direccion' => 'required|string|max:255',
            'telefono' => 'required|string|unique:productos,telefono,'.$this->route('cliente')->id.'|max:9',

            'correo' => 'required|email:rfc,dns|string|unique:productos,correo,'.$this->route('cliente')->id.'|max:255',
        ];
    }
